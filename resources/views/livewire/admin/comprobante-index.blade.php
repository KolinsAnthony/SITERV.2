                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <!-- Select para elegir el semestre -->
                            <select class="form-control" id="semestre" name="semestre" required onchange="toggleEgresadoInput(this)">
                            ...
                            <!-- Campo de texto que aparecerá cuando se seleccione EGRESADO -->
                            <input type="text" id="egresadoInput" name="semestre" class="form-control" style="display: none;" oninput="prefixEgresado(this)" maxlength="13">
                        </div>
                    </div>
...
<script>
    function toggleCategoryCheckboxes(source) {
     ...
     <script>
        ...
        <script>
            function toggleEgresadoInput(select) {
                const egresadoInput = document.getElementById('egresadoInput');
                
                // Muestra el campo de texto y oculta el select cuando se selecciona EGRESADO
                if (select.value === 'EGRESADO') {
                    egresadoInput.style.display = 'block';
                    egresadoInput.value = 'EGRESADO ';  // Agrega el prefijo EGRESADO al campo de texto
                    select.style.display = 'none';
                }
            }
        
            function prefixEgresado(input) {
                // Asegura que el campo siempre comience con "EGRESADO "
                if (!input.value.startsWith("EGRESADO ")) {
                    input.value = "EGRESADO " + input.value.replace(/^EGRESADO\s*/, ''); // Reemplaza prefijos diferentes
                }
        
                // Permite solo números después de "EGRESADO "
                const yearPart = input.value.replace("EGRESADO ", '');
                const numericYear = yearPart.replace(/\D/g, ''); // Remueve caracteres no numéricos
        
                // Limita la parte numérica a 4 dígitos
                if (numericYear.length > 4) {
                    input.value = "EGRESADO " + numericYear.slice(0, 4);
                } else {
                    input.value = "EGRESADO " + numericYear;
                }
            }
        </script>